...
IFACES = ["ibp63s0"]
READ_INTERVAL = 1.0
BUCKET_NS = 1_000_000  # 1ms buckets

CSV_PATH = "nic_usage.csv"  # 输出文件路径（相对/绝对均可）
...
def main():
    ...
    with open(CSV_PATH, "a", newline="") as fcsv:
        ...
        if write_header:
            writer.writerow(["bucket_ms", "iface", "tx_Mbps", "util"])
            fcsv.flush()
        ...
        while True:
            ...
            for bucket in sorted(agg.keys()):
                for idx, byte_cnt in agg[bucket].items():
                    ...
                    # CSV 追加写入
                    writer.writerow([int(bucket), idx_to_name[idx], f"{tx_mbps:.2f}", f"{util:.6f}"])
